...
class StudentRepository {
    ...
    companion object {
        ...
        fun searchStudentInfoByName(searchStudentName: String): StudentModel? {
            var studentModel: StudentModel? = null

            val studentList = getStudentInfo()

            if (studentList != null && studentList.size > 0) {
                for (item: StudentModel in studentList) {
                    if (item.studentName.equals(searchStudentName)) {
                        studentModel = item
                        break
                    }
                }
    ...
    fun checkStudentExistByName(searchStudentName:String):Boolean{
        var result:Boolean = true

        var studentList = getStudentInfo()
        if(studentList != null && studentList.size > 0){
            for (item : StudentModel in studentList){
                if(item.studentName.equals(searchStudentName)){
                    result = true
